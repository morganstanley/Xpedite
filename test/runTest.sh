...
function runPytests() {
  if [ "${PMC}" ]; then
    ...
    PYTHONPATH=${XPEDITE_DIR} pytest ${COV} ${TEST_NAME} -v ${APP_HOST} ${TRANSACTION_COUNT} ${THREAD_COUNT} ${WORKSPACE}
  else
    # omit test with performance counters
    PYTHONPATH=${XPEDITE_DIR} pytest ${COV} ${TEST_NAME} -v ${APP_HOST} ${TRANSACTION_COUNT} ${THREAD_COUNT} ${WORKSPACE} -m 'not pmc'
  fi
